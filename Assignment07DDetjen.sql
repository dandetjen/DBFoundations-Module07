...
-- Change Log: 
-- 2023-02-26,Dan Detjen,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_DDetjen')
	 Begin 
	  Alter Database [Assignment07DB_DDetjen] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_DDetjen;
	 End
	Create Database Assignment07DB_DDetjen;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_DDetjen;
...
--create function
Create Function dbo.fProduct_Inventory()
Returns Table
As
	Return(
		Select Top 100000
...
Select * From dbo.fProduct_Inventory()
go
...
Select	ProductName
		, Format (InventoryDate, 'MMMM, yyyy') As InventoryDate
		, Sum(Count) As InventoryCount
		, IsNull (Lag(Sum(Count)) Over (Partition By ProductName Order By InventoryDate), 0) As PreviousMonthInventoryCount
From vProducts As p
Inner Join vInventories As i
 On p.ProductID =  i.ProductID
...
-- create view 
-- Note:  could not use previous vProductInventories view.  The date data was in vchar, not Date format.  
--Could not figure out how change format.

Create View vProductInventoriesWithPreviouMonthCounts With SchemaBinding 
As 
...
Select
		...
		, InventoryDate
		, InventoryCount
		, PreviousMonthInventoryCount
		, (Case 
			When InventoryCount > PreviousMonthInventoryCount Then '1'
			When InventoryCount = PreviousMonthInventoryCount Then '0'
			When InventoryCount < PreviousMonthInventoryCount Then '-1'
